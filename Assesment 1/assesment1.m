...
% Sort both lists explicitly - filesystem ordering is not guaranteed
...
%  STEP 6: Fill Holes
%  =========================================================
% The parachute has multiple coloured panels separated by darker seams.
% These appear as internal black voids in the binary mask. Filling them
% produces one solid connected region per candidate object, which makes
% solidity and area measurements more reliable in the selection step.
...
% is identified using a two-stage process:
%
%   Stage 1 - Size filter: blobs covering > 25% of the image are
%             background regions, not the parachute.
%
%   Stage 2 - Scoring: mean_saturation x solidity x position_weight
%             The parachute is colourful (high S), compact (high
%             solidity) and always airborne (upper half of frame).
...
% by the global Otsu threshold. When the selected blob is under 2.5% of
% image area, a local re-threshold at 60% of the global Otsu value is
% applied within an expanded bounding box to recover missing pixels.
%
% Two safety checks guard against expanding into background:
%   1. Refined blob must be strictly larger than the original.
%   2. Refined blob mean saturation must not drop below 85% of original.
...
for i = 1:numImages
    ...
    % Safety check 1: must grow the mask
    if sum(refined_full(:)) <= sum(BW(:))
        mask_refined{i} = BW;
        continue;
    end

    % Safety check 2: mean saturation must not drop below 85% of original
    orig_mean_sat    = mean(S(find(BW)));           %#ok<FNDSB>
    refined_mean_sat = mean(S(find(refined_full))); %#ok<FNDSB>
    if refined_mean_sat < 0.85 * orig_mean_sat
        mask_refined{i} = BW;
        continue;
    end
...
% Ground truth loads as RGB so channel 1 is extracted and cast to logical.
